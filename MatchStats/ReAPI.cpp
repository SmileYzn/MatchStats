...
bool ReAPI_Init()
{
	...
	g_RehldsHookchains->Cvar_DirectSet()->registerHook(ReAPI_Cvar_DirectSet);
...
bool ReAPI_Stop()
{
	...
	g_RehldsHookchains->Cvar_DirectSet()->unregisterHook(ReAPI_Cvar_DirectSet);
...
void ReAPI_ClientConnected(IRehldsHook_ClientConnected* chain, IGameClient* client)
{
	...
	if (client)
	{
		auto NetworkUserID = client->GetNetworkUserID();

		if (NetworkUserID)
		{
			if (NetworkUserID->m_SteamID)
			{
				//LOG_CONSOLE(PLID, "[%s] %I64u", __func__, NetworkUserID->m_SteamID);
			}
		}
	}
}
...
void ReAPI_SV_DropClient(IRehldsHook_SV_DropClient* chain, IGameClient* client, bool crash, const char* Reason)
{
	...
	if (!FNullEnt(pEdict))
	{
		...
		if (Player)
		{
			gMatchStats.PlayerDisconnect(Player);
		}
	}
...
void ReAPI_Cvar_DirectSet(IRehldsHook_Cvar_DirectSet* chain, struct cvar_s* var, const char* value)
{
	chain->callNext(var, value);

	gMatchStats.Cvar_DirectSet(var, value);
}